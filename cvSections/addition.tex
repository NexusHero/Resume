\section{Projekte}

\cventry
{TRUMPF Laser Gmbh} % Education Title
{CAD/CAM-Prozesse} % Organization
{Schramberg, DE} % Location
{02/2024 - heute} % Date
\begin{cvitems}
	\item Entwicklung des CAD/CAM-Backends
	\item Einsatz von einer eigenen Somain Specific Language (DSL) in Kombination mit LionWeb in C\# abzubilden
	\item Implementierung von Unit tests
	\item[] \textbf{Frameworks/Tools: Softwarer-Architektur, DSL, LionWeb, VisualStudio, Azure Devops, Unit tests, C\#, .NET8, Scrum}
\end{cvitems}

\cventry
{TRUMPF Laser Gmbh} % Education Title
{Entwicklung eines Kommunikations-Frameworks } % Organization
{Schramberg, DE} % Location
{08/2023 - heute} % Date
\begin{cvitems}
	\item Scrum-Master- und Planungstätigkeit
	\item Anlegen von User stories
	\item Feature- und Aufwandsplanungen
	\item Implementierung und testing von Features
	\item[] \textbf{Frameworks/Tools: Softwarer-Architektur, C\#, VisualStudio, .NET6, gRPC, Scrum,  Gtest, Conan, Cmake, gtest, NUnit, Azure devops}
\end{cvitems}

\cventry
{TRUMPF Laser Gmbh} % Education Title
{Entwicklung und Integration einer Kameraplattform} % Organization
{Schramberg, DE} % Location
{04/2022 - 06/2023} % Date
\begin{cvitems}
	\item Iterative Erarbeitung und Implementierung eines pluginbasierte Software-Architektur
	\item Entwicklung von Treibersoftware für Baumer und MatrixVision
	\item Erstellung einer Dokumentation im Rahmen von ISAQB (Arc42)
	\item Implementierung von Unit-, Integrations- und Benchmarktests
	\item[] \textbf{Frameworks/Tools: Softwarerarchitektur, C++ (17), GenICam, Baumer, MatrixVision, Boost, Gtest, Conan, Cmake, gtest, Azure devops}
\end{cvitems}

\cventry
{TRUMPF Laser Gmbh} % Education Title
{Fullstack App zur Visualisierung von Prozessdaten} % Organization
{Schramberg, DE} % Location
{10/2015 - 02/2016} % Date
\begin{cvitems}
	\item Erarbeitung einer Software Architektur 
	\item Implementierung von Backend und Middleware basierend auf .NET-Framework
	\item[] \textbf{Frameworks/Tools: openCV, C\#, .NET-Framework, Lua, Axoom Cloud}
\end{cvitems}