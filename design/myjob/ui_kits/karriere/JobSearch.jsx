    ...
    <div onClick={() => onOpen(job.id)} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', minWidth: 0 }}>
          ...
          {missing.length > 0 && <span style={{ flexShrink: 0, display: 'inline-flex', alignItems: 'center', gap: '4px', fontFamily: 'var(--font-mono)', fontSize: '10px', fontWeight: 600, color: 'var(--text-soft)', background: 'var(--surface-subtle)', border: '1px solid var(--border)', borderRadius: 'var(--radius-pill)', padding: '2px 8px' }}><JS.Icon name="trend" size={10} />+{missing.length} new skill{missing.length === 1 ? '' : 's'}</span>}
        </div>
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 40, display: 'flex', justifyContent: 'flex-end' }}>
      ...
      <div style={{ position: 'relative', width: '480px', maxWidth: '92vw', height: '100%', background: 'var(--surface-card)', borderLeft: '1px solid var(--border)', boxShadow: 'var(--shadow-lg)', overflowY: 'auto', display: 'flex', flexDirection: 'column', animation: 'kslide var(--dur-med) var(--ease-out)' }}>
        <div style={{ padding: '22px 24px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'flex-start', gap: '14px' }}>
          ...
          <JS.IconButton icon="x" label="Close" variant="ghost" onClick={onClose} />
        </div>
        ...
        <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: '20px', flex: 1 }}>
          <section style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
            <KField label="Salary" value={job.salary} mono />
            <KField label="Country" value={job.country} />
            <KField label="Work model" value={job.mode} />
            <KField label="Posted" value={job.posted} />
          </section>
          <a href={job.url || undefined} target="_blank" rel="noopener noreferrer"
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: '9.5px', letterSpacing: 'var(--ls-wide)', textTransform: 'uppercase', color: 'var(--text-soft)' }}>Job posting{job.applyVia !== '—' ? ` · ${job.applyVia}` : ''}</div>
          ...
          <section>
            <h4 style={{ fontFamily: 'var(--font-mono)', fontSize: '10.5px', letterSpacing: 'var(--ls-wide)', textTransform: 'uppercase', color: 'var(--text-soft)', margin: '0 0 8px' }}>Description</h4>
        ...
        <div style={{ padding: '16px 24px', borderTop: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: '10px', background: 'var(--surface-subtle)', position: 'sticky', bottom: 0 }}>
          {job.url && (
            <a href={job.url} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none' }}>
              <JS.Button variant="outline" block iconLeft={<JS.Icon name="globe" size={15} />}>Open job posting</JS.Button>
            </a>
          )}
          <div style={{ display: 'flex', gap: '10px' }}>
            <JS.Button variant="primary" block iconLeft={<JS.Icon name="edit" size={15} />} onClick={() => onCreate(job)}>Create application</JS.Button>
            <JS.IconButton icon="bookmark" label="Save" variant="outline" size="lg" />
          </div>
        </div>
      </div>
    </div>
      ...
      <div style={{ maxWidth: '1100px' }}>
        <JS.Card>
          <div style={{ padding: '36px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '14px' }}>
            ...
            <div>
              <div style={{ fontFamily: 'var(--font-display)', fontSize: '17px', fontWeight: 700, color: 'var(--text-heading)' }}>Couldn't load jobs</div>
              <div style={{ fontSize: '13px', color: 'var(--text-muted)', marginTop: '4px', maxWidth: '460px' }}>Is the API server running? Enable job sources via <code style={{ fontFamily: 'var(--font-mono)' }}>JOB_SOURCES=arbeitnow,bundesagentur,adzuna</code>. ({error})</div>
            </div>
            <JS.Button variant="primary" iconLeft={<JS.Icon name="trend" size={15} />} onClick={runSearch}>Try again</JS.Button>
          </div>
        </JS.Card>
      </div>
    );
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '18px', maxWidth: '1100px' }}>
      <JS.Card pad={true} style={{ overflow: 'visible' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr 1.4fr auto', gap: '12px', alignItems: 'end' }}>
          <div>
            <label style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', letterSpacing: 'var(--ls-wide)', textTransform: 'uppercase', color: 'var(--text-soft)', display: 'block', marginBottom: '6px' }}>Country</label>
            <JS.Select options={COUNTRIES} value={country} onChange={(e) => setCountry(e.target.value)} />
          </div>
          <div>
            <label style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', letterSpacing: 'var(--ls-wide)', textTransform: 'uppercase', color: 'var(--text-soft)', display: 'block', marginBottom: '6px' }}>City / location</label>
            <div style={inputBox}><JS.Icon name="pin" size={15} style={{ color: 'var(--text-soft)' }} /><input value={city} onChange={(e) => setCity(e.target.value)} placeholder="e.g. Berlin" style={{ flex: 1, minWidth: 0, border: 'none', outline: 'none', background: 'transparent', fontFamily: 'var(--font-body)', fontSize: '13.5px', color: 'var(--text-heading)' }} /></div>
          </div>
          <div>
            <label style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', letterSpacing: 'var(--ls-wide)', textTransform: 'uppercase', color: 'var(--text-soft)', display: 'block', marginBottom: '6px' }}>Keywords</label>
            <div style={inputBox}><JS.Icon name="search" size={15} style={{ color: 'var(--text-soft)' }} /><input value={kw} onChange={(e) => setKw(e.target.value)} placeholder="Role, skill, company …" style={{ flex: 1, minWidth: 0, border: 'none', outline: 'none', background: 'transparent', fontFamily: 'var(--font-body)', fontSize: '13.5px', color: 'var(--text-heading)' }} /></div>
          </div>
          <JS.Button variant="primary" size="lg" iconLeft={<JS.Icon name="search" size={16} />} onClick={runSearch}>Search</JS.Button>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap', marginTop: '14px', paddingTop: '14px', borderTop: '1px solid var(--border)' }}>
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', letterSpacing: 'var(--ls-wide)', textTransform: 'uppercase', color: 'var(--text-soft)', marginRight: '2px' }}>Sources</span>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: '12px', color: 'var(--text-muted)' }}><span style={{ color: 'var(--text-heading)', fontWeight: 600 }}>{loading ? '…' : results.length}</span> Jobs · <span style={{ color: 'var(--status-hired-strong)', fontWeight: 600 }}>{loading ? '…' : topJobs.length}</span> top matches across {activeSources.size} source{activeSources.size === 1 ? '' : 's'}</div>
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: '10px' }}>
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', fontFamily: 'var(--font-mono)', fontSize: '10.5px', color: 'var(--text-soft)' }}><span style={{ width: '7px', height: '7px', borderRadius: '50%', background: 'var(--status-hired)' }} />live</span>
          <JS.Button size="sm" variant={onlyTop ? 'primary' : 'outline'} iconLeft={<JS.Icon name="zap" size={14} />} onClick={() => setOnlyTop((v) => !v)}>Top matches only</JS.Button>
          <JS.Button size="sm" variant="outline" iconLeft={<JS.Icon name="trend" size={14} />} onClick={runSearch}>Refresh</JS.Button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <JS.Card pad={false}><div style={{ padding: '48px', textAlign: 'center', color: 'var(--text-soft)', fontSize: '13px' }}>No jobs for this search. Adjust the filters or sources.</div></JS.Card>
      ) : (
        <React.Fragment>
          {topJobs.length > 0 && (
            <JS.Card pad={false}>
              <TierHeader icon="zap" tone="hired" title="Top matches" hint={`≥ ${THRESHOLD}% of your skills`} count={topJobs.length} />
              {topJobs.map((j) => <JobRow key={j.id} job={j} onOpen={setOpenJob} />)}
            </JS.Card>
          )}
          {!onlyTop && moreJobs.length > 0 && (
            <JS.Card pad={false}>
              <TierHeader icon="trend" tone="accent" title="More & growth opportunities" hint="new domains & technologies" count={moreJobs.length} />
              {moreJobs.map((j) => <JobRow key={j.id} job={j} onOpen={setOpenJob} stretch={true} />)}
            </JS.Card>
          )}
          {topJobs.length === 0 && onlyTop && (
            <JS.Card pad={false}><div style={{ padding: '40px', textAlign: 'center', color: 'var(--text-soft)', fontSize: '13px' }}>No matches ≥ {THRESHOLD}%. Turn off "Top matches only" to see growth opportunities.</div></JS.Card>
          )}
        </React.Fragment>
      )}
