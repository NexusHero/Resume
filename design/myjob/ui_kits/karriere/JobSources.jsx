...
const AUTH_LABEL = { public: 'Offene API', apikey: 'API-Schlüssel', oauth: 'OAuth' };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', padding: '18px', borderRadius: 'var(--radius-lg)', background: 'var(--surface-card)', border: `1px solid ${p.connected ? 'var(--accent-border)' : 'var(--border)'}`, boxShadow: 'var(--shadow-sm)' }}>
      ...
      <div style={{ display: 'flex', gap: '8px', borderTop: '1px solid var(--border)', paddingTop: '14px' }}>
        {p.connected ? (
          <React.Fragment>
            <SR.Button size="sm" variant="outline" iconLeft={<SR.Icon name="settings" size={14} />}>Verwalten</SR.Button>
            <SR.Button size="sm" variant="ghost" onClick={() => onDisconnect(p.id)}>Trennen</SR.Button>
          </React.Fragment>
        ) : (
          <SR.Button size="sm" variant="primary" block iconLeft={<SR.Icon name="plus" size={14} />} onClick={() => onConnect(p)}>Verbinden</SR.Button>
        )}
      </div>
    </div>
  );
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      ...
      <div style={{ position: 'relative', width: '480px', maxWidth: '100%', background: 'var(--surface-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius-xl)', boxShadow: 'var(--shadow-lg)', overflow: 'hidden', animation: 'kpop var(--dur-med) var(--ease-out)' }}>
        <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: '13px' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontFamily: 'var(--font-display)', fontSize: '17px', fontWeight: 700, color: 'var(--text-heading)' }}>{provider.name} verbinden</div>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: '11px', color: 'var(--text-soft)' }}>{AUTH_LABEL[provider.auth]} · {provider.region}</div>
          </div>
          <SR.IconButton icon="x" label="Schließen" variant="ghost" onClick={onClose} />
        </div>
        ...
        <div style={{ padding: '22px 24px', display: 'flex', flexDirection: 'column', gap: '18px' }}>
          {provider.auth === 'public' && (
            <div style={{ display: 'flex', gap: '11px', padding: '14px', borderRadius: 'var(--radius-md)', background: 'var(--status-hired-soft)', border: '1px solid var(--status-hired-border)' }}>
              ...
              <div style={{ fontSize: '12.5px', color: 'var(--text-body)' }}>Offene API — keine Zugangsdaten nötig. Mit einem Klick verbinden.</div>
            </div>
          )}
          {isKey && (
            <React.Fragment>
              <SR.Input label="App ID (optional)" icon="id" placeholder="z. B. a1b2c3" value={appId} onChange={(e) => setAppId(e.target.value)} />
              <SR.Input label="API-Schlüssel" icon="id" type="password" placeholder="••••••••••••••••" value={key} onChange={(e) => setKey(e.target.value)} hint={`Im ${provider.name}-Entwicklerportal erstellen. Wird verschlüsselt gespeichert.`} />
            </React.Fragment>
          )}
          {isOauth && (
            <div style={{ display: 'flex', gap: '11px', padding: '14px', borderRadius: 'var(--radius-md)', background: 'var(--accent-soft)', border: '1px solid var(--accent-border)' }}>
              ...
              <div style={{ fontSize: '12.5px', color: 'var(--text-body)' }}>Du wirst zu {provider.name} weitergeleitet, um myJob den Zugriff zu erlauben. Es werden keine Passwörter gespeichert.</div>
            </div>
          )}
          <label style={{ display: 'flex', alignItems: 'center', gap: '9px', fontSize: '12.5px', color: 'var(--text-muted)' }}>
            <SR.Icon name="info" size={14} style={{ color: 'var(--text-soft)' }} />Jobs werden alle 30 Minuten automatisch synchronisiert.
          </label>
        </div>
        ...
        <div style={{ padding: '16px 24px', borderTop: '1px solid var(--border)', background: 'var(--surface-subtle)', display: 'flex', justifyContent: 'flex-end', gap: '10px' }}>
          <SR.Button variant="ghost" onClick={onClose} disabled={busy}>Abbrechen</SR.Button>
          <SR.Button variant="primary" disabled={!canSubmit || busy} onClick={submit} iconLeft={<SR.Icon name={busy ? 'clock' : isOauth ? 'logout' : 'check'} size={15} />}>
            {busy ? 'Verbinde …' : isOauth ? `Mit ${provider.name} anmelden` : 'Verbinden'}
          </SR.Button>
        </div>
      </div>
    </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', maxWidth: '1100px' }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '14px' }}>
        <SR.StatCard label="Verbundene Quellen" value={connected.length} icon="globe" />
        <SR.StatCard label="Jobs aus APIs" value={totalJobs} icon="briefcase" delta="live" dir="up" />
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 18px', borderRadius: 'var(--radius-lg)', background: 'var(--surface-card)', border: '1px solid var(--border)' }}>
        ...
        <div style={{ flex: 1, fontSize: '12.5px', color: 'var(--text-muted)' }}>Verbinde Jobbörsen und Aggregatoren, um Stellen direkt in die <strong style={{ color: 'var(--text-heading)' }}>Jobsuche</strong> zu holen. Öffentliche Quellen verbinden sofort; andere brauchen einen Schlüssel oder OAuth.</div>
        <SR.Button size="sm" variant="outline" iconRight={<SR.Icon name="arrowRight" size={13} />} onClick={onFindJobs}>Zur Jobsuche</SR.Button>
      </div>

      <div>
        <h3 style={{ fontFamily: 'var(--font-display)', fontSize: '15px', fontWeight: 700, color: 'var(--text-heading)', margin: '0 0 12px' }}>Verfügbare Quellen</h3>
