...
> **[arc42](https://arc42.org)** architecture documentation for **myJob**. Modeling
> language: **UML** (PlantUML sources in [`docs/umls/`](umls), rendered to `.svg`).
> **This file is the single entry point** — chapters 1–12 are the architecture proper;
> the documentation map below links every companion artifact around it.
...
Product → [requirements](requirements.md) + [roadmap](roadmap.md). Every requirement is
walkable to its feature, runtime flow and ADR via the [traceability matrix](#traceability-matrix).
