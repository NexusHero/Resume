...
| `SELF_SERVE_TENANTS`                                        | _(off)_                                                    | `true` makes each new registration its own isolated workspace/tenant (admin of it); off keeps the single shared team (ADR-0036).        |
| `SUPER_ADMIN_EMAIL`                                         | —                                                          | Comma-separated emails granted the instance-level super-admin console (`/admin/*`, cross-tenant). Not grantable via the API (ADR-0037). |
