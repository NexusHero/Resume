...
| 0.9 | **Clean-code review pass** — the codebase grew fast; audit for SRP, naming, duplication, dead code and controller/service boundaries; refactor with no behaviour change, coverage held. Done: three-dimension audit (clean code / architecture / UX), runLlm scaffold collapse, candidateFacts unification, PII/sample-data removal, DE→EN sweep, naming sweep, Editor split. | `server/src/**`, recruiting kit          | M      | ✅                                    |
