...
export class FsPasswordResetTokenStore implements PasswordResetTokenStore {
  ...
  constructor(deps: { config: AppConfig; clock: Clock }) {
    this.file = deps.config.passwordResetTokensFile;
    this.dir = path.dirname(this.file);
    this.clock = deps.clock;
    this.ttlMs = deps.config.mail.resetTokenTtlMs;
  }

  private async readAll(): Promise<PasswordResetToken[]> {
    try {
  ...
  private async write(tokens: PasswordResetToken[]): Promise<void> {
