...
export class CandidacyController {
  ...
  board = async (req: Request, res: Response): Promise<void> => {
    res.json(await this.service.board(currentUserId(req), req.params.id as string));
  };

  /** POST /mandates/:id/candidacies — add a talent to the pipeline. */
  add = async (req: Request, res: Response): Promise<void> => {
    const input = addCandidacySchema.parse(req.body);
    const candidacy = await this.service.add(currentUserId(req), req.params.id as string, input);
  ...
  forTalent = async (req: Request, res: Response): Promise<void> => {
    res.json(await this.service.forTalent(currentUserId(req), req.params.id as string));
  };

  /** PATCH /candidacies/:id — move stage / reorder / annotate. */
  update = async (req: Request, res: Response): Promise<void> => {
    const patch = updateCandidacySchema.parse(req.body);
    const candidacy = await this.service.update(currentUserId(req), req.params.id as string, patch);
    res.json({ candidacy });
  };
