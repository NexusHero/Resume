...
This repo already implements §1.2 (MADR-format ADRs with supersede), §1.5 (`docs:check` is exactly
...
2. **CI-blocking coverage gate (§3.4).** `npm test` runs with `--coverage`, but CONTRIBUTING.md's
   "≥ 90%" is currently aspirational, not enforced. Wire a coverage threshold into the Jest config
   (or a CI step reading the coverage summary) so a PR that drops below it fails the same way a
   broken test does.
